  ...
  return (
    <CmtMediaObject
      ...
      {item.status === 'TECH_ASSIGNED' && (
        <IconButton aria-label="Start" className={classes.greenIcon} onClick={() => onBreakdownClick('START_M', item)}>
          <StartIcon />
        </IconButton>
      )}
      ...
      {item.status === 'IN_PROGRESS' && (
        <IconButton aria-label="Complete" className={classes.redIcon} onClick={() => onBreakdownClick('COMPLETE_M', item)}>
          <StopIcon />
        </IconButton>
      )}
    </CmtMediaObject>
