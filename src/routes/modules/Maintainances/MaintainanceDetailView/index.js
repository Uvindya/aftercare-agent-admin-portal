...
const MaintainanceDetailView = ({ open, onCloseDialog }) => {
  ...
  const { id, description, product,additionalNote, completionNote, targetCompletionDate ,maintainanceType, reportedAt, scheduledDate, status} = detailedCurrentMaintainance;
    ...
    <Dialog open={open} onClose={onCloseDialog} className={classes.dialogRoot}>
          ...
          <Box mt={-2}>
            <Box display="flex" alignItems="center">
              ...
              <Box mt={1}>
                ...
                { scheduledDate && <Typography className={classes.subTitleRoot}>{`Scheduled At : ${scheduledDate}`}</Typography> }
                { targetCompletionDate && <Typography className={classes.subTitleRoot}>{`Target Completion Date : ${targetCompletionDate}`}</Typography> }
      ...
      <Box px={6} py={5}>
        ...
        <Box mt={1}>
            ...
            <Typography className={classes.subTitleRoot}>{`Maintainnance Interval : ${product.maintainnanceInterval}`}</Typography>
        </Box>
      </Box>
      <Box px={6} py={5}>
        <Box mb={5} component="p" color="common.dark">
